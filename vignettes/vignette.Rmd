---
title: "CovidMutations: a feasible framework for annotation and mutation analysis of COVID-19"
author:
- name: Shaoqian Ma
  affiliation: 
...
bibliography: [bibliography.bib, url.bib]
vignette: >
  %\VignetteIndexEntry{CovidMutations: a feasible framework for annotation and mutation analysis of COVID-19}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
nocite: | 
  @RN206, @RN205
abstract: >
  Fundamental aspects of SARS-CoV-2 biology remain to be described, having the potential to provide insight to the response effort for this high-priority pathogen. Here we describe several aspects of mutational characteristics of SARS-CoV-2, including the geographical distribution, mutations density across the genome, mutation counts for each gene and proteins and evaluation of effectiveness of designed assays (presenting the mutational ratio). We believe that the analytic tools for dessecting sequence information throughout the SARS-CoV-2 pandemic will provide valuable insights into public health and genomic epidemiology.
---
...
# Function Overview of CovidMutations
...
```{r MutByGene, fig.cap = "Mutation counts for each gene", echo=TRUE, fig.height=13, fig.width=17, message=FALSE, warning=FALSE, paged.print=FALSE}
data("gene_position")
#outdir <- tempdir()
...
The detection of SARS-CoV-2 is important for the prevention of the outbreak and management of patients. Real-time reverse-transcription polymerase chain reaction (RT-PCR) assay is one of the most effective molecular diagnosis strategies to detect virus in clinical laboratory[@Hirotsu.2020].

## Calculate the mutation detection rate using different assays

The `assays` example data contains some current assays for detecting mutations across the SARS-CoV-2 genome. The "F1", "F2", "R1", "R2" refer to genomic positions of two pairs of primers (forward primer and reverse primer). The "P1","P2" refer to genomic positions of probes for detection. As SARS-CoV-2 genome is still changing over time, we assume that assays for detecting mutations should also be changed and optimized. Users can design their own assay and transform the assays information according to the format shown by the example data to evaluate whether their assays are efficient for detecting interesting mutations.
...
The function `AssayMutRatio` is to use the well established assays information to detect mutations in different SARS-CoV-2 genomic sites. The output will be series of figures presenting the mutation profile using a specific assay (output as file) and a figure for comparison between the mutation detection rate by each assay (if `outdir` is NULL, it returns only the figure for comparison).
...
If `outdir` is specified, one of the output figure of assays for mutation detection is like:

![](ChinaCDC-N.png)

## Bacth assay analysis for last five Nr of primers

Last five nucleotides of primer mutation count/type for any PCR (Polymerase Chain Reaction) primer. This is also an evaluation aspect for assays efficiency.
...
```{r LastfiveNrMutation, fig.cap = "Mutation detection counts (last five nucleotides for each primer) using different assays", echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
totalsample <- 4000
...
Moreover, researchers may have a question about the performance of double assays in detecting co-occurring mutations in the same samples with significant mutational pattern, as some mutations are definitely co-occurring more frequently. Here we designed a function for simultaneously measuring the mutated samples using two different assays. The information of first assay and second assay should be  structured like the example assays data, containing F1, F2, R1, R2, P1, P2 sites.  
...
The co_Mutation_Ratio is calculated by dividing the number of samples with co-occurring mutations by the mutated samples detected using each assay.
